...
![../_images/snakesAndLadders_2x.png](../_images/snakesAndLadders_2x.png)
...
Like the body of an `if` statement, each `case` is a separate branch of code execution. The `switch` statement determines which branch should be selected. This procedure is known as _switching_ on the value that is being considered.
...
> Although `break` is not required in Swift, you can use a `break` statement to match and ignore a particular case or to break out of a matched case before that case has completed its execution. For details, see [Break in a Switch Statement](#ID139).
...
For readability, a compound case can also be written over multiple lines. For more information about compound cases, see [Compound Cases](#ID548).

**Note**

> To explicitly fall through at the end of a particular `switch` case, use the `fallthrough` keyword, as described in [Fallthrough](#ID140).
...
![../_images/coordinateGraphSimple_2x.png](../_images/coordinateGraphSimple_2x.png)
...
![../_images/coordinateGraphMedium_2x.png](../_images/coordinateGraphMedium_2x.png)
...
![../_images/coordinateGraphComplex_2x.png](../_images/coordinateGraphComplex_2x.png)
...
The `continue`, `break`, and `fallthrough` statements are described below. The `return` statement is described in [Functions](Functions.xhtml), and the `throw` statement is described in [Propagating Errors Using Throwing Functions](ErrorHandling.xhtml#ID510).
